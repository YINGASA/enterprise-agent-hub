  ...
  return (
    <div>
      <PageHeader
        ...
        title="聊天工作台"
        description="展示场景选择、mock 对话、Agent 决策过程、工具调用记录与结构化 JSON 输出。当前版本不接入真实对话。"
      />
        ...
        <aside className="rounded-lg border border-slate-200 bg-white p-4 shadow-sm">
          ...
          <div className="mt-4 space-y-3">
            {scenarios.map((scenario, index) => (
              <div
                key={scenario.id}
                className={`rounded-lg border p-3 ${index === 1 ? "border-brand-200 bg-brand-50" : "border-slate-200 bg-white"}`}
              >
        ...
        <section className="rounded-lg border border-slate-200 bg-white p-5 shadow-sm">
          <div className="mb-5 flex items-center justify-between">
            <div>
              ...
              <p className="text-sm text-ink-500">电商客服与售后 Agent 示例</p>
            </div>
