...
function responseModeLabel(mode: AgentApiResponse["api"]["responseMode"] | LlmMode) {
  const labels: Record<string, string> = { mock: "Mock 模式", real: "Real API 成功", real_repaired: "Real API · JSON 修复", real_text_fallback: "Real API · 文本兜底", real_error_fallback: "Real API 失败，已使用兜底回答", fallback: "兜底模式" };
  ...
  return (
    <div className="mx-auto w-full max-w-7xl space-y-5 overflow-x-hidden">
      <section className="rounded-lg border border-slate-200 bg-white p-6 shadow-sm">
        <div className="min-w-0 space-y-4">
          <div>
            ...
            <p className="mt-2 rounded-lg border border-slate-200 bg-slate-50 p-4 text-sm leading-6 text-ink-600">你可以自由输入问题，不限于示例；系统会通过 Agent Router 判断场景，并决定是否调用 RAG、业务工具和 Real API / Mock 模式。线上演示默认推荐 Mock 模式，可稳定体验完整流程。</p>
          </div>
          <div className="grid max-w-sm grid-cols-2 gap-2 rounded-lg bg-slate-100 p-1 text-sm ring-1 ring-slate-200">
          ...
          <div className={healthResult?.healthy ? "rounded-md border border-emerald-200 bg-emerald-50 p-3 text-sm leading-6 text-emerald-800" : llmStatus?.configured ? "rounded-md border border-amber-200 bg-amber-50 p-3 text-sm leading-6 text-amber-800" : "rounded-md border border-slate-200 bg-slate-50 p-3 text-sm leading-6 text-ink-700"}>
          ...
          <textarea value={question} onChange={(event) => setQuestion(event.target.value)} rows={6} className="w-full resize-y rounded-lg border border-slate-300 bg-white px-4 py-3 text-sm leading-7 text-ink-800 outline-none transition focus:border-brand-500 focus:ring-2 focus:ring-brand-100" placeholder={"\u8f93\u5165\u4f01\u4e1a\u5236\u5ea6\u3001\u552e\u540e\u3001\u62db\u8058\u6216 AI \u5de5\u7a0b\u95ee\u9898\uff0c\u4f8b\u5982\uff1a\u8ba2\u535510001\u80fd\u4e0d\u80fd\u9000\uff1f"} />
          <div className="flex flex-col gap-3 sm:flex-row">
            <button type="button" onClick={handleRun} disabled={runButtonDisabled} className="min-h-11 cursor-pointer rounded-md bg-brand-600 px-6 py-2.5 text-sm font-semibold text-white transition-colors hover:bg-brand-700 disabled:cursor-not-allowed disabled:bg-slate-400 disabled:text-white">{isLoading ? loadingMessage + "..." : realApiUnavailable ? "Real API 未启用，请使用 Mock" : "\u751f\u6210\u56de\u7b54"}</button>
            <button type="button" onClick={handleHealthCheck} disabled={isCheckingHealth} className="min-h-11 cursor-pointer rounded-md border border-slate-200 bg-white px-5 py-2.5 text-sm font-semibold text-ink-700 transition-colors hover:border-brand-200 hover:bg-brand-50 hover:text-brand-700 disabled:cursor-not-allowed disabled:border-slate-200 disabled:bg-slate-100 disabled:text-ink-500">{isCheckingHealth ? "检查中..." : "检查 LLM 连接"}</button>
          </div>
      ...
      <section className="rounded-lg border border-slate-200 bg-white p-5 shadow-sm">
        ...
        {result ? <div className="mt-5 grid gap-3 sm:grid-cols-2 xl:grid-cols-4"><div className="rounded-md border border-slate-200 bg-white p-3"><p className="text-xs text-ink-500">业务场景 / 任务意图</p><p className="mt-1 break-words text-sm font-semibold text-ink-900">{scenarioLabel(result.route.scenario)} / {intentLabel(result.route.intent)}</p></div><div className="rounded-md border border-slate-200 bg-white p-3"><p className="text-xs text-ink-500">置信度 / 风险等级</p><p className="mt-1 text-sm font-semibold text-ink-900">{Math.round(result.route.confidence * 100)}% / {riskLabel(result.structuredOutput.riskLevel)}</p></div><div className="rounded-md border border-slate-200 bg-white p-3"><p className="text-xs text-ink-500">兜底回答 / 来源引用</p><p className="mt-1 text-sm font-semibold text-ink-900">{usedFallback ? "是" : "否"} / {result.ragAnswer?.sources.length ?? 0} 条</p></div><div className="rounded-md border border-slate-200 bg-white p-3"><p className="text-xs text-ink-500">调用工具</p><p className="mt-1 break-words text-sm font-semibold text-ink-900">{formatTools(result.structuredOutput.toolsUsed)}</p></div></div> : null}
      </section>
      ...
      <section className="space-y-4">
        <CollapsibleSection title="\u67e5\u770b\u8fd0\u884c Trace" description="Router\u3001RAG\u3001Tools\u3001Retriever \u4e0e\u7ed3\u6784\u5316\u8f93\u51fa\u9ed8\u8ba4\u6536\u8d77\uff0c\u9700\u8981\u590d\u76d8\u65f6\u518d\u5c55\u5f00\u3002" defaultOpen={false}><AgentTracePanel result={result} /></CollapsibleSection>
        {healthResult ? <CollapsibleSection title="\u67e5\u770b LLM \u8fde\u63a5\u8bca\u65ad" description="\u4ec5\u5c55\u793a\u8131\u654f\u540e\u7684\u8fde\u63a5\u72b6\u6001\u548c\u9519\u8bef\u7c7b\u578b\uff0c\u4e0d\u66b4\u9732\u6a21\u578b\u670d\u52a1\u914d\u7f6e\u3002" defaultOpen={false}><pre className="max-h-[420px] max-w-full overflow-x-auto overflow-y-auto whitespace-pre-wrap break-words rounded-md bg-slate-950 p-4 text-xs leading-6 text-slate-100">{JSON.stringify(healthResult, null, 2)}</pre></CollapsibleSection> : null}
        <CollapsibleSection title="\u67e5\u770b\u5b8c\u6574\u6765\u6e90\u5f15\u7528" description="Top sources \u4e4b\u5916\u7684\u5b8c\u6574\u5f15\u7528\u5217\u8868\u9ed8\u8ba4\u6536\u8d77\uff0c\u907f\u514d\u5e72\u6270\u4e3b\u56de\u7b54\u3002" defaultOpen={false}><SourceList sources={result?.ragAnswer?.sources ?? []} /></CollapsibleSection>
      </section>

      <CollapsibleSection title="Chat \u8fd0\u884c\u5386\u53f2" description="\u4fdd\u5b58\u5355\u6b21 Agent \u8fd0\u884c Trace\uff0c\u7528\u4e8e\u590d\u76d8\u548c\u5bfc\u51fa\uff1b\u4e0e\u5f53\u524d\u56de\u7b54\u4e3b\u6d41\u7a0b\u5206\u5f00\u5448\u73b0\u3002" defaultOpen={false}><ChatRunHistoryPanel currentResult={result} /></CollapsibleSection>
    </div>
  );
}
