      ...
      <section className="min-w-0 space-y-5">
        <div className="rounded-lg border border-slate-200 bg-white p-6 shadow-sm">
          <div className="flex flex-wrap items-start justify-between gap-3">
            ...
            <button type="button" onClick={handleClearUserDocuments} disabled={!userDocuments.length} className="cursor-pointer rounded-md border border-rose-200 px-3 py-2 text-xs font-semibold text-rose-600 transition-colors hover:bg-rose-50 disabled:cursor-not-allowed disabled:border-slate-200 disabled:text-ink-400">{ui.clearUserDocs}</button>
          </div>
          <div className="mt-5 grid gap-3 text-sm sm:grid-cols-2 xl:grid-cols-4"><div className="rounded-md border border-slate-200 bg-slate-50 p-4"><p className="text-xs text-ink-500">{ui.defaultDocCount}</p><p className="mt-1 font-semibold text-ink-900">{defaultDocuments.length} {ui.docUnit}</p></div><div className="rounded-md border border-slate-200 bg-slate-50 p-4"><p className="text-xs text-ink-500">{ui.userDocCount}</p><p className="mt-1 font-semibold text-ink-900">{userDocuments.length} {ui.docUnit}</p></div><div className="rounded-md border border-slate-200 bg-slate-50 p-4"><p className="text-xs text-ink-500">{ui.totalChunks}</p><p className="mt-1 font-semibold text-ink-900">{defaultChunkCount + userChunkCount} {ui.chunkUnit}</p></div><div className="rounded-md border border-slate-200 bg-slate-50 p-4"><p className="text-xs text-ink-500">{ui.activeSources}</p><p className="mt-1 break-words font-semibold text-ink-900">{activeSourceTypes.join(" / ")}</p></div></div>
