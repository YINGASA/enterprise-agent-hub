  ...
  return <div className="space-y-6 overflow-x-hidden">
    <section className="rounded-lg border border-slate-200 bg-white p-6 shadow-sm">
      <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
        <div>
          <h2 className="text-lg font-semibold text-ink-900">V1.7 Agent 质量评测看板</h2>
          <p className="mt-1 text-sm leading-6 text-ink-500">用于持续验证 Agent Router、Hybrid RAG、Tool Calling、fallback 与结构化输出质量。运行结果可保存为本地历史，查看趋势图表，并预览或导出 Markdown / JSON 报告。</p>
        </div>
        <button type="button" onClick={runEvaluation} disabled={isRunning} className="min-h-11 cursor-pointer rounded-md bg-brand-600 px-6 py-2.5 text-sm font-semibold text-white transition-colors hover:bg-brand-700 disabled:cursor-not-allowed disabled:bg-slate-400">{isRunning ? "评测中..." : "运行评测"}</button>
      </div>
    ...
    </> : <p className="rounded-lg border border-dashed border-slate-300 bg-white p-8 text-center text-sm leading-6 text-ink-500">请选择测试集规模并运行评测。评测结果、历史记录和报告导出操作会显示在这里。</p>}
