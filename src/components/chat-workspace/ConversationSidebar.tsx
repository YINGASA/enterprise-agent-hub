  ...
  return (
    <div className="flex h-full min-h-0 flex-col bg-white">
      <div className="shrink-0 border-b border-slate-200 p-4">
        <button type="button" data-testid="conversation-new" onClick={() => { onNewConversation(); onSelected?.(); }} className="w-full cursor-pointer rounded-lg bg-brand-600 px-4 py-2.5 text-sm font-semibold text-white transition-colors hover:bg-brand-700 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand-500 focus-visible:ring-offset-2">新建对话</button>
      ...
      <div data-testid="conversation-list" className="min-h-0 flex-1 space-y-2 overflow-y-auto p-3" aria-label="会话列表">
        {filtered.length ? filtered.map((conversation) => {
          const selected = conversation.id === activeConversationId;
          const renaming = conversation.id === renamingId;
          return (
            <article key={conversation.id} data-testid="conversation-list-item" className={"group rounded-lg border p-2 transition-colors " + (selected ? "border-brand-200 bg-brand-50" : "border-transparent hover:border-slate-200 hover:bg-slate-50")}>
              {renaming ? (
                <div>
                  <label>
                ...
                <>
                  <button type="button" data-testid="conversation-select" aria-current={selected ? "page" : undefined} onClick={() => selectConversation(conversation.id)} className="w-full cursor-pointer rounded-md px-1 py-1 text-left focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand-500">
                    <span data-testid="conversation-title" className="block truncate text-sm font-semibold text-ink-800">{conversation.title}</span>
                  ...
                  <div className="mt-1 flex items-center gap-1 px-1 lg:opacity-0 lg:transition-opacity lg:group-hover:opacity-100 lg:group-focus-within:opacity-100">
                    <button type="button" data-testid="conversation-rename" aria-label={`重命名会话 ${conversation.title}`} onClick={() => beginRename(conversation)} className="cursor-pointer rounded px-2 py-1 text-xs font-medium text-ink-600 hover:bg-white hover:text-brand-700 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand-500">重命名</button>
                    <button type="button" data-testid="conversation-delete" aria-label={`删除会话 ${conversation.title}`} onClick={() => onRequestDelete(conversation.id)} className="cursor-pointer rounded px-2 py-1 text-xs font-medium text-rose-700 hover:bg-rose-50 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-rose-500">删除</button>
                  </div>
                </>
              )}
            </article>
    ...
    <>
      ...
      {props.mobileOpen ? (
        <div className="fixed inset-0 z-[60] lg:hidden" role="dialog" aria-modal="true" aria-labelledby="mobile-conversation-title">
          ...
          <aside ref={drawerRef} tabIndex={-1} id="mobile-conversation-drawer" data-testid="conversation-drawer" className="relative h-full w-[min(88vw,320px)] border-r border-slate-200 bg-white shadow-2xl">
