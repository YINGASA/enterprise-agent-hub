  ...
  return (
    <div className="min-w-0 space-y-4">
      <CollapsibleSection title="Streaming" description="只展示流式模式、降级、停止和事件数量等安全标量，不展示原始事件载荷。" defaultOpen={false}>
        <MockJsonPanel title="流式安全摘要" data={{ requestAction: result.api.requestAction ?? "send", streamingRequested: result.api.streamingRequested === true, streamingUsed: result.api.streamingUsed === true, streamFallback: result.api.streamFallback === true, aborted: result.api.aborted === true, deltaCount: Number.isInteger(result.api.streamDeltaCount) ? Math.max(0, result.api.streamDeltaCount ?? 0) : 0, durationMs: Number.isFinite(result.api.llmDurationMs) ? Math.max(0, Math.floor(result.api.llmDurationMs ?? 0)) : 0 }} />
      </CollapsibleSection>
