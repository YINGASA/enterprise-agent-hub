...
export type KnowledgeQualityAssessment = {
  ...
  label: "优秀" | "可用" | "需补充";
  dimensions: { content: number; tags: number; testQuestions: number; chunks: number; ragEnabled: number };
  issues: string[];
};
...
export type DuplicateKnowledgeMatch = { documentId: string; title: string; similarity: number; reasons: string[] };
...
export function assessKnowledgeDocument(document: KnowledgeDocument, chunksInput = splitDocument(document)): KnowledgeQualityAssessment {
  ...
  const chunkCount = chunksInput.length;
...
export function diagnoseKnowledgeChunks(chunks: KnowledgeChunk[]): ChunkQualityDiagnostic[] {
  return chunks.map((chunk, index) => {
    ...
    return { chunkId: chunk.id, characterCount, status: issues.length ? "warning" : "good", issues };
  });
}

export function findDuplicateKnowledgeDocuments(candidate: Pick<KnowledgeDocument, "title" | "content">, documents: KnowledgeDocument[]): DuplicateKnowledgeMatch[] {
  ...
  return documents
    .map((document) => {
      ...
      return { documentId: document.id, title: document.title, similarity: Math.round(Math.max(exactTitle ? 1 : titleSimilarity, contentSimilarity) * 100), reasons };
    })
