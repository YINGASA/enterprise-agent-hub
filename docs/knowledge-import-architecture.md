...
- `process` 每次最多 claim 一个项目，并使用随机 claim token、租约和条件更新防止两个处理器重复消费。
...
- 每个 Workspace 最多保留 20 个活动任务和 50 个带临时提取正文的项目；超过 24 小时的未确认预览会在下一次预览前清理。单批解析并发固定为 2；单服务进程同时最多 4 个预览、同一 Workspace 最多 1 个预览。
