...
- 基于 Next.js + TypeScript 构建 Enterprise Agent Hub 企业级 AI Agent 平台，聚焦企业制度与流程问答、电商客服售后、AI 工程规范和可追踪评测。
...
- 构建 Agent Evaluation Dashboard，覆盖 80 条多场景测试用例，统计场景识别、意图识别、工具命中、RAG 引用、关键词命中和 fallback 等指标，Mock 完整评测通过率 100%。
...
- 实现本地规则版 Agent Router，根据用户问题自动识别 enterprise / ecommerce / ai_engineering / general 场景与知识问答、售后政策、订单查询、商品查询、工单创建等意图。
- 实现 mock RAG 流程：文档录入、文本切片、关键词提取、TopK 召回、来源引用和 mock 回答，为后续接入 Embedding 与向量数据库预留边界。
- 实现本地 Tool Calling 层：订单查询、商品查询、政策检索、工单创建、客服回复生成，并在 Agent Trace 中展示每一步输入、输出和耗时。
