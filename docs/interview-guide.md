...
我做了一个 Enterprise Agent Hub，用 Next.js 和 TypeScript 搭了一个企业级 Agent 平台原型。它支持企业知识库问答、电商客服售后和招聘 JD 匹配三个场景。用户输入后，系统会先用 Agent Router 判断场景和意图，再决定是否走 RAG、调用哪些业务工具，最后可以用 DeepSeek 这类 OpenAI-compatible 模型生成结构化 JSON。如果模型 JSON 不稳定，会做一次 repair，再失败就用真实文本兜底。项目还内置了 15 条评测集和 Evaluation Dashboard，用来统计路由准确率、工具命中率、RAG 引用率、关键词命中率和 fallback 率。这个项目重点展示的是 AI 应用工程化闭环，而不是单纯调一次模型 API。