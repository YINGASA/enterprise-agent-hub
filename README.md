...
V2.2.1 在 V2.2.0 PostgreSQL 服务端知识存储基础上增加工作区隔离的企业知识包、多文件导入预览、安全文件解析、重复检测、冲突处理、分块质量诊断和可恢复导入任务。成功项按文档独立事务写入，失败项可重试，页面刷新后可以从服务端恢复任务进度。

数据库层继续使用 Prisma 6.19.3 与 PostgreSQL。服务端以签名的不透明 HttpOnly Cookie 解析匿名工作区，所有知识包、任务、文档和 chunks 查询均限定 `workspaceId`。这提供浏览器级隔离基础，但不等同于完整登录、账号恢复或企业多租户认证。

V2.1.0 的 Context Plan、Rolling Summary、Streaming / Revision Safety 和 V2.2.0 的会话持久化语义保持不变。本版本不保存原始上传文件，不接入对象存储，不支持 OCR；扫描型 PDF 无可提取文本时会返回明确提示。
...
- 导入安全：扩展名、MIME、文件签名和实际结构共同校验，限制批次、文件、提取正文、PDF 页数、DOCX 解压规模与解析并发；单服务进程预览总并发为 4，同 Workspace 为 1。
