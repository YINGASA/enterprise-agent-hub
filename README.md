...
## V2.0.4 Quality Baseline

V2.0.4 focuses the active product portfolio on enterprise knowledge, policy and process queries, ecommerce service workflows, RAG, business tools, and traceable evaluation. Retired scenario identifiers remain readable only for historical browser and Ops records, while user-imported documents are never removed by product-scenario changes. The release baseline is 29 unit-test files with 116/116 passing tests, 28/28 Playwright E2E scenarios, and Full Mock Evaluation at 80/80 with a 100% pass rate.
