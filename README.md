...
V2.2.0 在 V2.1.0 Conversation Context Manager 基础上增加可选的 PostgreSQL 服务端存储。会话、消息、滚动摘要、用户知识文档、知识分块和迁移记录通过统一 Repository 接口访问；数据库未启用时继续兼容原有 localStorage，数据库已启用但暂时不可用时进入明确的 degraded 模式，不会静默回写本地造成双数据源分叉。

数据库层使用 Prisma 6.19.3 与 PostgreSQL。服务端以签名的不透明 HttpOnly Cookie 解析匿名工作区，所有业务查询和写入均限定 `workspaceId`。这提供浏览器级隔离基础，但不等同于完整登录、账号恢复或企业多租户认证。

V2.1.0 的 Context Plan、Smart Context Selection、Rolling Summary、Streaming Summary Patch、Regenerate / Edit & Resend、CAS 与 Completion Fence 语义保持不变。Mock 与 Real 继续共用同一 Context Plan；滚动摘要仍由确定性逻辑生成，没有增加第二次摘要模型调用。
...
- 服务端存储：Prisma 6.19.3 + PostgreSQL 持久化 Conversation、Message、Conversation Summary、KnowledgeDocument、KnowledgeChunk、ImportJob 与迁移状态。
...
- 服务端存储健康时，用户导入或粘贴的文本知识文档及其 chunks 保存在当前匿名工作区的 PostgreSQL 数据中，RAG 只读取该工作区已启用的文档。
...
- 本版本支持受限大小的文本正文写入 PostgreSQL，不接入对象存储，也不增加知识包批量导入 UI。
...
- 当前没有对象存储、向量数据库、长期记忆、跨会话记忆或用户画像。
...
- 对象存储与文档解析：支持 PDF / DOCX / OCR、大文件与知识包批量导入。
