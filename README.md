...
## V2.1.0 Conversation Context Manager

V2.1.0 adds a server-authoritative Conversation Context Manager for long conversations. It combines a deterministic token budget, recent-turn preservation, rules-based selective history, optional rolling summaries and safe context traces into one Context Plan shared by Mock, Real, streaming and non-streaming execution. V2.0.4 scenario-retirement compatibility remains intact: legacy records are readable, while retired scenarios are not reactivated.

The local release baseline is 36 unit-test files with 148/148 passing tests, 29/29 Playwright E2E scenarios, and Full Mock Evaluation at 80/80 with a 100% pass rate. V2.1.0 has not been pushed, tagged, merged or deployed.
