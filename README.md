...
V2.2.3 在 V2.2.2 生产加固基线上统一企业工作台设计语言，优化 Chat、知识库与企业知识包导入、评测中心、运行监控、业务工具、首页和 About 的信息层级、状态反馈、响应式与键盘可用性。本版本不增加大型业务功能，也不改变 Router、RAG、Tool Calling、Real API、上下文管理、Streaming、CAS、数据库 Schema 或导入任务语义。

项目实际使用仓库内 `redesign-existing-projects` Skill 完成审查，但只采纳视觉层级、间距、组件一致性、状态、响应式和无障碍建议。外部字体、营销式大标题、巨大留白、玻璃拟态、夸张渐变、破格布局和滚动动效均不适用于高信息密度企业工作台，因此没有采用。

运行基线保持 Node 20.19.5、Prisma 6.19.3 与 `undici@7.28.0`。PostgreSQL 匿名工作区、Context Plan、Rolling Summary、Streaming / Revision Safety、企业知识包和持久化导入任务继续保持原有语义；没有新增 migration。本版本不保存原始上传文件，不接入对象存储，不支持 OCR。腾讯云现已运行 V2.2.3，部署 commit 为 `739fa7dd54a3565d6e04f424ef8a40692c611c4a`；生产当前使用 `local` 存储模式，数据库未配置，未执行生产 PostgreSQL migration。
...
- 知识库问答：默认 Knowledge Packs 与当前存储模式下的工作区文档共同参与 RAG。
...
- `/about`：当前产品定位、系统模块、工作流程、版本能力、安全边界与推荐演示路径。
...
- `/evaluation`：Agent 评测中心，支持 80 条 Full Mock 评测、失败定位、历史趋势和 Markdown / JSON 报告。
...
- 腾讯云当前运行 V2.2.3，部署 commit 为 `739fa7dd54a3565d6e04f424ef8a40692c611c4a`；生产使用 `local` 存储模式，数据库未配置，未执行生产 PostgreSQL migration。
...
- `docs/v2.2.3-ui-guidelines.md`：V2.2.3 设计 Token、页面状态、响应式、无障碍和 UI 测试规范。
...
- `docs/v2.2.3-release-notes.md`：V2.2.3 界面体验与产品质感优化发布说明。
