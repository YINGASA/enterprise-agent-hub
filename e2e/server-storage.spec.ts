...
test("迁移按 preview、确认、execute 执行，重复标记阻止再次导入且保留本地备份", async ({ page }) => {
  ...
  const uploadDialog = page.getByRole("alertdialog", { name: "确认上传本地数据？" });
  await expect(uploadDialog).toBeVisible();
  await uploadDialog.getByRole("button", { name: "允许上传并预检" }).click();

  const executeDialog = page.getByRole("alertdialog", { name: "确认执行数据迁移？" });
  await expect(executeDialog).toContainText("预检完成：已导入 2，已跳过 0，冲突 0，失败 0");
  await executeDialog.getByRole("button", { name: "执行迁移" }).click();
...
test("迁移在用户允许上传前不会发送本地正文", async ({ page }) => {
  ...
  await page.goto("/chat");
  const migrationButton = page.getByRole("button", { name: "迁移本地数据" });
  await migrationButton.click();
  const uploadDialog = page.getByRole("alertdialog", { name: "确认上传本地数据？" });
  await expect(uploadDialog).toBeVisible();
  await expect(uploadDialog.getByRole("button", { name: "取消" })).toBeFocused();
  await page.keyboard.press("Escape");
  await expect(uploadDialog).toBeHidden();
  await expect(migrationButton).toBeFocused();
