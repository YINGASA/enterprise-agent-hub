...
function makeItem(index: number, duplicateType: ImportItem["duplicateType"]): ImportItem {
  ...
  return {
    ...
    chunkPreview: [{
      chunkIndex: 0,
      characterCount: 220,
      approximateTokens: 110,
      keywords: ["制度", "员工", "差旅"],
      contentPreview: `${title}的受限分块预览，不包含完整文件正文。`,
      tooShort: false,
      tooLong: false,
...
test("server 模式完成知识包、多文件预览、元数据编辑、失败重试与刷新恢复", async ({ page }) => {
  ...
  await expect(firstItem).toContainText("受限分块预览");
  const secondItem = page.getByTestId("knowledge-import-item-item-2");
  await secondItem.getByLabel("冲突处理").selectOption("import_as_new");
